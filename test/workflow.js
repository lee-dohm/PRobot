...
describe('Workflow', () => {
  describe('matches', () => {
    it('is truthy for matching event', () => {
      ...
      expect(workflow.matches({event: {event: 'issues', payload: {}}})).toBeTruthy();
    });

    it('is truthy for multiple events', () => {
      const workflow = new Workflow(['issues', 'pull_request']);
      expect(workflow.matches({event: {event: 'pull_request', payload: {}}})).toBeTruthy();
    });

    it('is truthy for event with action', () => {
      const workflow = new Workflow(['issues.opened']);
      expect(
        workflow.matches({event: {event: 'issues', payload: {action: 'opened'}}})
      ).toBeTruthy();
    });
    ...
    it('is truthy for multiple events with action', () => {
      ...
      expect(
        workflow.matches({event: {event: 'issues', payload: {action: 'labeled'}}})
      ).toBeTruthy();
    });

    it('is falsy for different event', () => {
      const workflow = new Workflow(['issues']);
      expect(workflow.matches({event: {event: 'pull_request', payload: {}}})).toBeFalsy();
    });
